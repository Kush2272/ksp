...
pub fn run_proxy(listen: &str, upstream: &str, json: bool) {
    let listen_addr: SocketAddr = match listen.parse() {
        ...
        Err(_) => match format!("{}:8080", listen).parse() {
            ...
            Err(e) => {
                if !json {
                    ui::failure(&format!("Invalid listen address '{}': {}", listen, e));
                }
                return;
            }
        },
                            ...
                            tokio::select! {
                                ...
                                n = tcp_socket.read(&mut buf) => {
                                    match n {
                                        Ok(0) | Err(_) => break,
                                        Ok(bytes_read) => {
                                            if client.send_packet(PacketType::Data, sid, &buf[..bytes_read]).await.is_err() {
                                                break;
                                            }
                                    ...
                                    match pkt_res {
                                        Ok((pkt, payload)) => {
                                            if pkt.stream_id == sid || pkt.stream_id == 0 {
                                                if tcp_socket.write_all(&payload).await.is_err() {
                                                    break;
                                                }
...
pub fn run_gateway(listen: &str, target_http: &str, json: bool) {
    let listen_addr: SocketAddr = if listen.contains(':') {
        match listen.parse() {
            ...
            Err(_) => {
                if !json {
                    ui::failure(&format!("Invalid KSP listen address '{}'", listen));
                }
                return;
            }
        }
        ...
        match format!("{}:9876", listen).parse() {
            ...
            Err(_) => {
                if !json {
                    ui::failure(&format!("Invalid KSP listen port '{}'", listen));
                }
                return;
            }
        }
