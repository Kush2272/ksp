...
pub fn run_create(name: &str, json: bool) {
    ...
    if path.exists() {
        if json {
            println!(
                "{}",
                serde_json::json!({"error": "profile_exists", "name": name})
            );
        } else {
            println!(
    ...
    if json {
        ...
        println!(
            "  {} Created profile '{}' with default tokens.",
            "✔".green().bold(),
            name.white().bold()
        );
        println!(
            "  {} Switch to it using `ksp profile switch {}`.",
            "ℹ".blue(),
            name
        );
    }
...
pub fn run_switch(name: &str, json: bool) {
    ...
    if json {
        println!(
            "{}",
            serde_json::json!({"status": "switched", "active_profile": name})
        );
    } else {
        println!(
            "  {} Switched active KSP profile -> {}",
            "✔".green().bold(),
            name.white().bold()
        );
    }
...
pub fn run_list(json: bool) {
    ...
    if json {
        println!(
            "{}",
            serde_json::json!({"profiles": profiles, "active": active})
        );
        return;
    }
