    ...
    match KspPacket::deserialize(&data) {
        Ok((packet, consumed)) => {
            if json {
                ...
                t.add_row(vec![
                    "Session ID",
                    &uuid::Uuid::from_bytes(packet.session_id).to_string(),
                ]);
...
pub fn run_decode(file: &str, json: bool) {
    ...
    if data.len() >= 48 {
        if !json {
            ...
            ui::kv(
                "Flags",
                &format!("0x{:04X}", u16::from_be_bytes([data[2], data[3]])),
            );
            ui::kv(
                "Payload length",
                &format!(
...
pub fn run_build(output: &str, json: bool) {
    ...
    let payload = if let Some(stripped) = payload_input.strip_prefix("0x") {
        hex::decode(stripped).unwrap_or_else(|_| payload_input.as_bytes().to_vec())
    } else {
...
pub fn run_export(file: &str, format: &str, json: bool) {
    ...
    if json {
        println!(
            "{}",
            serde_json::json!({"file": file, "format": format, "exported": exported})
        );
        return;
    }
...
pub fn run_visualize(file: &str, json: bool) {
    ...
    if let Ok(data) = std::fs::read(file)
        && let Ok((pkt, _)) = KspPacket::deserialize(&data)
    {
        println!(
            "  File: {} ({} total wire bytes)\n",
            file.yellow().bold(),
            data.len()
        );
        println!(
        ...
        println!(
            "  │ {:<26} │ {:<29} │",
            "48 Byte Fixed Header".white().bold(),
            "Protocol Metadata".dimmed()
        );
        println!(
            "  {}",
            "├────────────────────────────┼─────────────────────────────┤".cyan()
        ...
        println!(
            "  │ {:<26} │ {:<29} │",
            "Version [Offset 0:1]",
            format!("v{}", pkt.version).green()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "Type [Offset 1:2]",
            pkt.packet_type.name().yellow()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "Flags [Offset 2:4]",
            pkt.flags.to_string().cyan()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "Session ID [Offset 4:20]",
            format!("..{}", hex::encode(&pkt.session_id[12..16])).white()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "Stream ID [Offset 20:24]",
            format!("#{}", pkt.stream_id).green()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "Sequence # [Offset 24:32]",
            format!("#{}", pkt.sequence).yellow()
        );
        println!(
            "  │ {:<26} │ {:<29} │",
            "AEAD Nonce [Offset 32:48]",
            format!("..{}", hex::encode(pkt.nonce.get(12..16).unwrap_or(&[]))).dimmed()
        );
        println!(
            "  {}",
            "├────────────────────────────┼─────────────────────────────┤".cyan()
        ...
        println!(
            ...
            format!("{} bytes (AES-256-GCM)", pkt.payload.len())
                .white()
                .bold()
        );
        println!(
        ...
        println!(
            ...
            if pkt.auth_tag.is_empty() {
                "None (Plaintext)".into()
            } else {
                format!("..{}", hex::encode(pkt.auth_tag.get(12..16).unwrap_or(&[])))
            }
        );
        println!(
            "  {}",
    ...
    println!(
        "  │ {:<26} │ {:<29} │",
        "Version [Offset 0:1]", "v1.0 (8-bit integer)"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "Type [Offset 1:2]", "Handshake / Data / Alert"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "Flags [Offset 2:4]", "ENCRYPTED | FIN | RST | ACK"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "Session ID [Offset 4:20]", "128-bit UUID (16 bytes)"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "Stream ID [Offset 20:24]", "32-bit Logical Stream (4B)"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "Sequence # [Offset 24:32]", "64-bit Replay Counter (8B)"
    );
    println!(
        "  │ {:<26} │ {:<29} │",
        "AEAD Nonce [Offset 32:48]", "128-bit IV/Salt (16 bytes)"
    );
    println!(
        "  {}",
        "├────────────────────────────┼─────────────────────────────┤".cyan()
    ...
    println!(
        "  │ {:<26} │ {:<29} │",
        "Encrypted Payload [Variable]", "Application Data Buffer"
    );
    println!(
        "  {}",
        "├────────────────────────────┼─────────────────────────────┤".cyan()
    ...
    println!(
        "  │ {:<26} │ {:<29} │",
        "AEAD Auth Tag [+16 bytes]", "128-bit GCM/Poly1305 Tag"
    );
    println!(
        "  {}",
        "└────────────────────────────────────────────────────────────┘".cyan()
