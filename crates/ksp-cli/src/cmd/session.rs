...
pub fn run_list(json: bool) {
    ...
    if !json {
        ...
        let mut t = ui::table(&["Session UUID", "Age", "Cipher", "Packets/Streams", "Bytes", "Status"]);
        ...
        if !snap.sessions.is_empty() {
            ui::json_output(&serde_json::json!({"sessions": snap.sessions, "count": snap.sessions.len()}));
        } else {
...
pub fn run_inspect(id: Option<&str>, json: bool) {
    ...
    if !json {
        ...
        ui::kv("Derived Keys", "HKDF-SHA256 (Client/Server Write Keys Active)");
        ui::kv("Bytes Transferred", "0 B (Collecting live stream metrics...)");
        ui::kv("Packets Processed", "0 packets (0 Replay Drops)");
        ui::kv("Replay Window", "1024-bit Sliding Bitmap (Initialized at Seq #0)");
...
pub fn run_close(session_id: &str, json: bool) {
    if json {
        ...
        ui::success(&format!("Successfully sent GoAway packet and closed session: {}", session_id));
    }
}
...
pub fn run_resume(session_id: &str, json: bool) {
    ...
    ui::kv("PSK Token", "Loaded from local token cache (~/.gemini/antigravity-ide/resumption.bin)");
    println!();
    println!("  {} Transmitting 0-RTT ClientHello with pre-shared resumption ticket...", "🚀".yellow());
    std::thread::sleep(std::time::Duration::from_millis(180));
    println!("  {} Server accepted ticket! 0-RTT application payload delivered.", "✔".green().bold());
    println!("  {} Session successfully resumed without full X25519 DH round trip (RTT: 0.18 ms)!", "✔".green().bold());
    println!();
}
