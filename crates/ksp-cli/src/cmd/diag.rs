...
pub fn run_diag(dump: bool, json: bool) {
    ...
    let os_info =
        sysinfo::System::long_os_version().unwrap_or_else(|| "Windows / OS Unknown".into());
    let total_mem = ui::format_bytes(sys.total_memory());
    let cpus = sys.cpus().len();

    let cert_exists =
        fs::metadata("certs/server.cert").is_ok() || fs::metadata("server.cert").is_ok();
    ...
    ui::kv(
        "KSP CLI Version",
        &format!("v{}", ksp_core::CURRENT_VERSION),
    );
    ...
    ui::kv(
        "TLS/PSK Certificate",
        if cert_exists {
            "certs/server.cert (Valid ✔)"
        } else {
            "Not found (Self-signed auto-gen)"
        },
    );
    println!();
    ...
    if dump {
        ...
        println!(
            "  {} Diagnostic state dumped to {}",
            "✔".green().bold(),
            dump_path.yellow().bold()
        );
        println!();
    } else {
        println!(
            "  {} Run `ksp diag --dump` to write full report to `ksp_diag_report.txt`.",
            "ℹ".blue()
        );
        println!();
    }
