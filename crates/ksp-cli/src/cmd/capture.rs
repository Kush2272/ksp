...
pub fn append_packet_to_pcap(packet_bytes: &[u8]) -> std::io::Result<()> {
    ...
    let mut file = OpenOptions::new().create(true).append(true).open(pcap_path)?;
...
pub fn run_start(port: u16, json: bool) {
    ...
    if let Err(e) = fs::write(&pcap_path, &PCAP_GLOBAL_HEADER) {
        if !json { ui::failure(&format!("Failed to write PCAP 2.4 header: {}", e)); }
        return;
    }
    let _ = fs::write(&pid_path, format!("{}", std::process::id()));

    // Record initial capture session event
    crate::cmd::telemetry::LogEntry::record("info", None, &format!("PCAP capture initialized on port {} (DLT 147 USER0)", port));

    // Append a synthetic or recorded handshake capture frame to initialize buffer
    let sample_pkt = KspPacket::new_handshake(ksp_core::types::PacketType::ClientHello, b"capture_start".to_vec());
    ...
    println!();
    println!("  {} Started live PCAP 2.4 recording engine on port {}", "✔".green().bold(), port.to_string().cyan());
    println!("  {} All KSP sessions & transfers will append PCAP records.", "ℹ".blue());
    println!("  {} Run `ksp capture stop` or `ksp capture export` when done.\n", "ℹ".blue());
}
...
pub fn run_stop(json: bool) {
    ...
    if let Ok(mut f) = File::open(&pcap_path) {
        ...
        if f.read_to_end(&mut buf).is_ok() && buf.len() > 24 {
            ...
            while offset + 16 <= buf.len() {
                let incl_len = u32::from_le_bytes([buf[offset+8], buf[offset+9], buf[offset+10], buf[offset+11]]) as usize;
    ...
    println!("  {} Finalized PCAP 2.4 buffer: {} ({} frames recorded)", "✔".green().bold(), ui::format_bytes(size), packets);
    println!("  {} Inspect live or export with `ksp capture export --format pcap`.\n", "ℹ".blue());
}
...
pub fn run_export(format: &str, output: &str, json: bool) {
    ...
    let data = match fs::read(&pcap_path) {
        ...
        Err(e) => {
            if json {
                ui::json_output(&serde_json::json!({"status": "error", "message": format!("Cannot read capture buffer: {}", e)}));
            } else {
    ...
    if format.eq_ignore_ascii_case("json") {
        ...
        let _ = fs::write(&out_file, serde_json::to_string_pretty(&json_out).unwrap_or_default());
    } else {
        let _ = fs::copy(&pcap_path, &out_file);
    }

    if json {
        ui::json_output(&serde_json::json!({"status": "exported", "format": format, "file": out_file}));
        return;
    }

    ui::success(&format!("Exported PCAP capture buffer to {} (Format: {})", out_file.white().bold(), format.cyan()));
    println!();
}

pub fn run_live(json: bool) {
    if json {
        ui::json_output(&serde_json::json!({"status": "error", "message": "Live mode requires interactive terminal"}));
        return;
    }
    ...
    loop {
        if let Ok(data) = fs::read(&pcap_path) {
            if data.len() > last_pos + 16 {
                while last_pos + 16 <= data.len() {
                    let ts_sec = u32::from_le_bytes([data[last_pos], data[last_pos+1], data[last_pos+2], data[last_pos+3]]);
                    let ts_usec = u32::from_le_bytes([data[last_pos+4], data[last_pos+5], data[last_pos+6], data[last_pos+7]]);
                    let incl_len = u32::from_le_bytes([data[last_pos+8], data[last_pos+9], data[last_pos+10], data[last_pos+11]]) as usize;
                    if last_pos + 16 + incl_len <= data.len() {
                        ...
                        if let Ok((pkt, _)) = KspPacket::deserialize(frame_bytes) {
                            ...
                            let info = format!("Stream #{:<3} | Flags: 0x{:02X} | Payload: {} B", pkt.stream_id, pkt.flags.bits(), pkt.payload.len());
                            println!("  {:<6} {} [{:<16}] {:<40}", format!("#{}", counter).dimmed(), ts_formatted.dimmed(), color_type, info);
                        }
