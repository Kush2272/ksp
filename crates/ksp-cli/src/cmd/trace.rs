...
pub fn run(message: Option<&str>, json: bool) {
    ...
    println!("  Tracing lifetime of packet: \"{}\"", payload.yellow().bold());
    println!();

    let steps = [
        ("Application Layer", format!("Initiate send for payload \"{}\" ({} bytes)", payload, payload.len()), "app.send()"),
        ("Stream Multiplexer", "Assign logical Stream ID #1, verify 64 KB flow control credit".into(), "stream.write()"),
        ("Session State", "Assign monotonically increasing Sequence #1042, derive Counter Nonce".into(), "session.encrypt()"),
        ("Packet Builder", "Assemble 48-byte fixed binary header (Version, Type, Flags, Seq, Nonce)".into(), "KspPacket::new()"),
        ("AEAD Encryption", "Execute AES-256-GCM encryption & append 16-byte cryptographic auth tag".into(), "aead::encrypt()"),
        ("Socket Transport", "Flush 80 total encrypted bytes directly into underlying TCP/TLS stream".into(), "TcpStream::write_all()"),
        ("Network Layer", "Wire transmission across physical or loopback network interface".into(), "IP/TCP Layer"),
        ("Decryption & Verify", "Receiver verifies AEAD auth tag, advances 1024-bit replay window bitmap".into(), "aead::decrypt()"),

        ("Application Delivery", format!("Payload verified & delivered intact to receiving application: \"{}\"", payload), "app.on_message()"),
    ];
