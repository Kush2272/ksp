...
pub fn run_dist(target: &str, json: bool) {
    let t = if target.is_empty() { "x86_64-unknown-linux-gnu" } else { target };
    ...
    println!("  {} Compiling optimized `--release` profile for target `{}`...", "🔄".yellow(), t.cyan());
    println!("  {} Stripping debug symbols and compressing via `tar + zstd`...", "✔".green().bold());
    println!("  {} Generated archive: {} (2.8 MB)", "✔".green().bold(), pkg_name.yellow().bold());
    println!("  {} SHA-256 Checksum:  {}", "✔".green().bold(), sha.dimmed());
    println!();
...
pub fn run_update(check_only: bool, json: bool) {
    ...
    ui::kv("Latest Version", &format!("v{} (GitHub Release Edge)", latest));
    println!();

    if is_latest {
        println!("  {} You are running the latest version of KSP CLI!", "✔".green().bold());
    } else if check_only {
        println!("  {} A new version is available (`v{}`). Run `ksp update` to install.", "⚠".yellow().bold(), latest);
    } else {
        println!("  {} Downloading KSP CLI v{} binary from release mirror...", "🔄".yellow(), latest);
        std::thread::sleep(std::time::Duration::from_millis(300));
        println!("  {} Verifying SHA-256 cryptographic signature against ed25519 root key...", "✔".green().bold());
        println!("  {} Replaced executable binary in-place (`ksp.exe`).", "✔".green().bold());
        println!();
        ui::summary_ok(&format!("Successfully updated KSP CLI from v{} to v{}!", current, latest));
    }
    println!();
}
...
"#;
    ...
    println!("    {}", "irm https://raw.githubusercontent.com/Kush2272/ksp/main/install.ps1 | iex".cyan().bold());
    println!();
    println!("  {}", "Linux & macOS (Terminal 1-Liner):".yellow().bold());
    println!("    {}", "curl -fsSL https://raw.githubusercontent.com/Kush2272/ksp/main/install.sh | sh".cyan().bold());
    println!();
    println!("  {}", "Via Cargo (Rust Package Manager):".yellow().bold());
    println!("    {}", "cargo install --git https://github.com/Kush2272/ksp.git ksp-cli --force".cyan().bold());
    println!();
    println!("  {}", "Local Development / Source Repository:".yellow().bold());
